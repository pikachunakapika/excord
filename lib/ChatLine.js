...
var ChatLine = GObject.registerClass(
    ...
    _init(user_id, avatar, username, text, jsonObj, guildObj, channelObj, owner) {
        ...
        this.connect("button-press-event", () => {
            if (channelObj) {
                owner.targetChannelId = channelObj.id;
                owner.indicator.chatTarget.set_text("#" + channelObj.name);
                Utils.loadIcon(guildObj.icon, 'https://cdn.discordapp.com/icons/' + guildObj.id + '/' + guildObj.icon + '.png?size=32', (gicon) => {
                    owner.indicator.guild.set_gicon(gicon);
                });
            }
            owner.indicator.channelSuggestion.close(BoxPointer.PopupAnimation.FULL)
        });
